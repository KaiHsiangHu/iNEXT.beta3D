...
<td align="left">a numerical vector specifying the diversity orders. Default is <code>q = c(0, 1, 2)</code>.</td>
...
<td align="left">a positive integer specifying the number of bootstrap replications when assessing sampling uncertainty and constructing confidence intervals. Bootstrap replications are generally time consuming. Set `nboot = 0` to skip the bootstrap procedures. Default is `nboot = 10`. If more accurate results are required, set `nbbot = 100 (or `nbbot = 200`).</td>
...
<td align="left">(argument only for <code>diversity = "PD"</code>), a numerical value specifying reference time for PD. Default is <code>PDreftime = NULL</code> (i.e., the age of the root of PDtree).</td>
...
<td align="left">	
(required argument only for <code>diversity = "FD"</code>), a species pairwise distance matrix for all species in the pooled assemblage.</td>
...
# Abundance data
output_TDc_abun = iNEXTbeta3D(data = Brazil_rainforests, diversity = 'TD', datatype = "abundance", 
...
```{r echo=FALSE}
output_TDc_abun = iNEXTbeta3D(data = Brazil_rainforests, diversity = 'TD', datatype = "abundance", base = 'coverage', nboot = 30)

# lapply(output_TDc_abun, function(x) 
#   
...
output_PDc_abun = iNEXTbeta3D(data = Brazil_rainforests, diversity = 'PD', datatype = "abundance", 
                              base = 'coverage', nboot = 30, PDtree = Brazil_tree)
output_PDc_abun
...
<td align="left"> <code>iNEXTbeta3D</code> object.</td>

</tr>
<tr class="even">
<td align="center"><code>type</code></td>
<td align="left">(argument only for `base = "coverage"` in <code>iNEXTbeta3D</code>), type = 'B' (default) for plotting coverage-based rarefaction and extrapolation sampling curves for gamma, alpha, and beta diversity; type = 'D' for plotting coverage-based four classes of dissimilarity indices; skip this argument for plotting sized-based rarefaction and extrapolation sampling curves for gamma and alpha diversity.  </td>
...
output_TDs_abun = iNEXTbeta3D(data = Brazil_rainforests, diversity = 'TD', datatype = 'abundance', 
...
data(Brazil_rainforests)
data(Brazil_distM)
output_FDc_abun = iNEXTbeta3D(data = Brazil_rainforests, diversity = 'FD', datatype = 'abundance', 
                              base = "coverage", nboot = 30, conf = 0.95, 
                              FDdistM = Brazil_distM, FDtype = 'AUC')
